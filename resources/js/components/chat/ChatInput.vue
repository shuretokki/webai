...
<template>
    <form @submit.prevent="submit" class="w-full max-w-[600px] shrink-0 relative flex items-end gap-2 p-2 bg-[#1e1e1e] border-l-4 border-[#abb463] rounded-none shadow-lg transition-all focus-within:ring-1 focus-within:ring-[#abb463]/50">
        ...
        <button
            type="submit"
            :disabled="!input.trim()"
            class="shrink-0 size-[48px] relative flex items-center justify-center rounded-none bg-gradient-to-br from-[#dbf156] to-[#acb564] text-black hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed transition-all overflow-hidden group"
        >
            <!-- Ripple Effect Background -->
            <div class="absolute inset-0 opacity-30">
                <div class="absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-white/50 via-transparent to-transparent scale-0 group-hover:scale-150 transition-transform duration-500"></div>
